...
#     )
#     ys = interp.interpolate_smooth(
#         nd_to_columns(y.astype('float64'), ndims, nrows, ncols).copy(),
#         indices=indices,
#         **smoothers.SMOOTH_KWARGS
#     )
...
# def multicolor_ylabel(
#     ax, list_of_strings, list_of_colors, axis='x', anchorpad=0, **kw
# ):
#     """
#     Reference:
...
#         boxes = [
#             TextArea(
#                 text, textprops=dict(color=color, ha='left', va='bottom', **kw)
#             )
...
#                 text,
#                 textprops=dict(
#                     color=color, ha='center', va='bottom', rotation=90, **kw
#                 ),
#             )
...
# def replace_upper_envelope(
#     y_array_sm, out_array_sm, col_count, smj, t, t_half, t_diff, t_adjust
# ):
...
#             sample_std = scale_min_max(
#                 clip_high(_get_array_std(ytest, j, j + t), 0.05),
#                 0.0,
#                 1.0,
#                 0.0,
#                 0.05,
...
#             t_adjust = int(
#                 scale_min_max(
#                     logistic_func(bcw_mu, mid_k, r_k), min_window, t, 0.0, 1.0
#                 )
#             )
...
#                             > ytest[j + t_diff]
#                         ):
#                             vc_scaled = scale_min_max(
#                                 -1.0 * vc_scaled, 0.1, 1.0, -1.0, 1.0
#                             )
#                             r_tadj = -r_t
#                         else:
#                             vc_scaled = scale_min_max(
#                                 vc_scaled, 0.1, 1.0, -1.0, 1.0
#                             )
#                             r_tadj = r_t

#                         tw = scale_min_max(
#                             logistic_func(vc_scaled, mid_t, r_tadj),
#                             0.1,
#                             1.0,
#                             0.0,
#                             1.0,
...
#                     else:
#                         tw = scale_min_max(
#                             gaussian_func(abs(vc_scaled), base_adjust),
#                             0.1,
#                             1.0,
#                             0.0,
#                             1.0,
...
#                             w_plot_w.append(
#                                 (
#                                     xinfo.dates_smooth[xoff + j + zz],
#                                     xinfo.dates_smooth[
#                                         xoff + j + zz + t_adjust - 1
#                                     ],
#                                 )
#                             )
...
#     OutlierData = namedtuple(
#         'OutlierData', 'col_pos xdata ydata alpha beta1 beta2'
#     )
...
#         if (
#             (low_values < yvalue <= baseline)
#             and (low_values < ydatav[0] <= baseline)
#             and (low_values < ydatav[2] <= baseline)
#         ):
#             outlier_count = 0
#         else:

#             yhat = linear_adjustment(
#                 int(xdatav[0]),
#                 int(xdatav[1]),
#                 int(xdatav[2]),
#                 ydatav[0],
...
#             if (
#                 (low_values < yvalue <= 0.4)
#                 and (max(ydatav[0], ydatav[2]) >= 0.45)
#                 and (ydatav[0] > yvalue < ydatav[2])
#             ):
#                 # Protect crop cycles
...
#             # Weight the distance
#             max_dist = max(
#                 abs(xdatav[1] - xdatav[0]), abs(xdatav[1] - xdatav[2])
#             )
#             scaled_days = 1.0 - scale_min_max(
#                 max_dist, 0.1, 0.9, 0.0, max_days
#             )

#             w2 = scale_min_max(
#                 logistic_func(scaled_days, 0.5, 10.0), 0.1, 1.0, 0.0, 1.0
#             )
...
#                     if (prop_diff(ydatav[0], ydatav[1]) < -0.9) and (
#                         prop_diff(ydatav[0], ydatav[2]) < -0.9
#                     ):
#                         outlier_count = 0

#                     # This check avoids large increases from valleys to green-up
#                     elif (prop_diff(ydatav[2], ydatav[1]) < -0.9) and (
#                         prop_diff(ydatav[2], ydatav[0]) < -0.9
#                     ):
#                         outlier_count = 0
#                     else:
...
#             yhat = clip(
#                 mbf[ii, 0] * xvalue + mbf[ii, 1] * pow(xvalue, 2) + mbf[ii, 2],
#                 0.0,
#                 1.0,
#             )
...
#             w2 = 1.0 - scale_min_max(
#                 abs(float(dhalf) - float(indexesv[jj])), 0.1, 1.0, 0.0, dmax
#             )
...
#                     # This check avoids large dropoffs from senescence to valleys
#                     if (prop_diff(ydatav[0], ydatav[2]) < -0.9) and (
#                         prop_diff(ydatav[0], ydatav[3]) < -0.9
#                     ):
#                         outlier_count += 0

#                     # This check avoids large increases from valleys to green-up
#                     elif (prop_diff(ydatav[4], ydatav[2]) < -0.9) and (
#                         prop_diff(ydatav[4], ydatav[1]) < -0.9
#                     ):
#                         outlier_count += 0
#                     else:
...
# def remove_outliers_linear(
#     xsparse, array_, mbz, ii, ncols, min_samples, max_days, dev_thresh, no_data
# ):
...
#                         max_dist1 = max(
#                             xsparse[end1] - xsparse[start], max_dist1
#                         )
#                         middle1 = end1
#                         increase_valid = True
...
#                         max_dist1 = max(
#                             xsparse[end1] - xsparse[middle1], max_dist1
#                         )
#                         middle1 = end1
#                         increase_valid = True
#                     else:
...
#                         max_dist2 = max(
#                             xsparse[end3] - xsparse[start], max_dist2
#                         )
#                         middle2 = end3
#                         increase_valid = True
...
#                         max_dist2 = max(
#                             xsparse[end3] - xsparse[middle2], max_dist2
#                         )
#                         increase_valid = True
#                         middle2 = end3
#                     else:
...
#             outlier_info, array_ = _check_deviation(
#                 indexes,
#                 xdata,
#                 ydata,
#                 array_,
...
# def remove_outliers_polynomial(
#     xsparse, array_, mbz, ii, ncols, min_samples, max_days, dev_thresh, no_data
# ):
...
#     outlier_info1, yseries_no_outliers = remove_outliers_linear(
#         xinfo.xd,
#         np.ascontiguousarray(
#             np.c_[yseries.flatten().copy(), yseries.flatten().copy()].T,
#             dtype='float64',
#         ),
...
#     outlier_info2 = remove_outliers_polynomial(
#         xinfo.xd,
#         np.ascontiguousarray(
#             np.c_[
#                 yseries_no_outliers.flatten().copy(),
#                 yseries_no_outliers.flatten().copy(),
#             ].T,
#             dtype='float64',
#         ),
...
#             yhat = np.clip(
#                 beta0 * np.cos(w * (float(j3) + 1))
#                 + beta1 * np.sin(w * (float(j3) + 1))
#                 + alpha,
#                 0.0,
#                 1.0,
#             )
