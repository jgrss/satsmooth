...
def get_time_slice(dates_array, start_dt, end_dt, skip):
    ...
    if skip == 'WMS':
        ...
        skip_idx = np.array(
            [
                dtidx
                for dtidx in range(0, dates_slice.shape[0])
                if (dates_slice[dtidx].timetuple().tm_mday == 1)
                or (dates_slice[dtidx].timetuple().tm_mday % 7 == 0)
            ],
            dtype='uint64',
        )
...
class ResampleDates(object):
    def __init__(self, indices, datetimes, rule='D'):
        ...
        self.series_raw = pd.Series(
            self.indices_raw, index=pd.DatetimeIndex(datetimes).round('D')
        )
        ...
        if rule.startswith('D') and (len(rule) > 1):
            ...
            self.series_resamp = self.series_raw.resample('D').interpolate(
                'linear'
            )
            ...
            skip_idx = np.array(
                [
                    dtidx
                    for dtidx in range(0, self.series_resamp.shape[0])
                    if (
                        self.series_resamp.index[dtidx]
                        .to_pydatetime()
                        .timetuple()
                        .tm_mday
                        < 30
            ...
            self.series_resamp = self.series_raw.resample(rule).interpolate(
                'linear'
            )

        # Get the smooth values
        self.indices_resamp = np.ascontiguousarray(
            self.series_resamp.values, dtype='float64'
        )
...
def prepare_x(X, start, end, rule='D', skip='WMS', write_skip=10):
    ...
    skip_orig_idx = np.array(
        [
            i
            for i in range(0, rd.series_raw.shape[0])
            if start_dt <= rd.series_raw.index[i].to_pydatetime() <= end_dt
        ],
        dtype='uint64',
    )

    py_dates_slice, start_idx, end_idx, skip_idx = get_time_slice(
        rd.dates_resamp, start_dt, end_dt, skip
    )
    ...
    # Indices for writing
    write_skip_idx = np.array(
        [
            dtidx
            for dtidx in range(0, skip_slice.shape[0])
            if (skip_slice[dtidx].timetuple().tm_mday < 30)
            and (
                (skip_slice[dtidx].timetuple().tm_mday == 1)
                or (skip_slice[dtidx].timetuple().tm_mday % write_skip == 0)
            )
        ],
        dtype='int64',
    )
        ...
        [
            ...
            and (
                abs(
                    ...
                    - xinfo.dates[i].timetuple().tm_yday
                )
                <= max_days
            )
        ],
