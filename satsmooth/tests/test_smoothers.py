...
from satsmooth.utils import nd_to_columns, columns_to_nd
...
def f(xdata):
    return rescale_intensity(np.sin(xdata), out_range=(0.2, 0.7)) + np.random.uniform(low=-0.05, high=0.05, size=xdata.shape[0])  #np.random.normal(scale=0.05, size=xdata.shape[0])


# y_in = np.random.random((2, xinfo.xd.shape[0]))
y_in_orig = np.array([rescale_intensity(np.sin(x), out_range=(0.2, 0.7)).tolist() for i in range(0, 2)]).clip(0, 1)
y_in = np.array([f(x).tolist() for i in range(0, 2)]).clip(0, 1)

nodata = 0

for i in range(0, y_in.shape[0]):
    idx = np.random.choice(range(0, y_in.shape[1]), size=int(0.3*y_in.shape[1]), replace=False)
    y_in[i, idx] = nodata

indices = np.ascontiguousarray(xinfo.skip_idx + xinfo.start_idx, dtype='uint64')

y = interpolator.interpolate_smooth(np.ascontiguousarray(y_in.copy(), dtype='float64'),
                                    fill_no_data=True,
...
smt = AncSmoothers(xinfo,
                   columns_to_nd(np.ascontiguousarray(y_in.copy(), dtype='float64'),
                                 xinfo.xd.shape[0],
                                 1,
                                 2),
                   pad=81,
