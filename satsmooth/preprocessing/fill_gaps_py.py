...
def scale_min_max(x, min_in, max_in, min_out, max_out):
    """Scales linearly"""
    return np.clip((((max_out - min_out) * (x - min_in)) / (max_in - min_in)) + min_out, min_out, max_out)
...
def fill_x(xdates, xdata, nodata):
    ...
    for k in range(1, nlyrs):
        ...
        day_weight = 1.0 - scale_min_max(day_diff, min_day_diff, max_day_diff, 0.0, 1.0)
...
def get_spatial_weights(w, hw):
    ...
    for m in range(0, w):
        for n in range(0, w):
            spatial_weights[m, n] = logistic_func(1.0 - ((edist(float(n), float(m), hw)+0.001) / max_edist), 0.5, 10.0)
...
def get_strides(array, w):
    ...
    strided_array = np.lib.stride_tricks.as_strided(array,
...
def get_masks(cluster_strides, cluster_centers):
    ...
    for index, r in np.ndenumerate(np.empty((h, w))):
        ...
        sl = (slice(index[0], index[0]+1),
              slice(index[1], index[1]+1),
              slice(0, None),
              slice(0, None))

        mask[sl] = np.where(cluster_strides[sl] == cluster_centers[index[0], index[1]], 1, 0)

    return mask


def lstsq(X_strides=None,
          y_strides=None,
          w_strides=None,
          X_centers=None,
          nodata=None):
    ...
    est = np.nan_to_num(beta1*X_centers + beta2*X_centers**2 + alpha,
                        nan=nodata, posinf=nodata, neginf=nodata)
...
def fill_gaps(data, clusters, w, nodata=0, dev_thresh=0.1):
    ...
    for b in range(0, y.shape[0]):
        ...
        X_centers = np_nanmean(X_strides[(slice(0, None),
                                          slice(0, None),
                                          slice(hw-1, hw+2),
                                          slice(hw-1, hw+2))], axis=(2, 3))

        # Squared deviation = (x - y)^2
        sq_diff = (X_strides - y_strides)**2

        # Mask where data is valid in both and the deviation is low
        valid_mask = np.where(~np.isnan(y_strides) &
                              ~np.isnan(X_strides) &
                              (X_strides != 0) &
                              (sq_diff <= dev_thresh), 1, 0)
        ...
        if valid_mask.max() == 1:
            ...
            estimates[b] = np.where(estimates[b] != 0,
                                    lstsq(X_strides=X_strides,
                                          y_strides=y_strides,
                                          w_strides=w_strides,
                                          X_centers=X_centers,
                                          nodata=nodata),
